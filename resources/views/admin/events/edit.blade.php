...
<div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm max-w-3xl">
    ...
    {{-- @method('PUT') WAJIB ada — browser hanya bisa GET/POST,
         direktif ini tambahkan hidden input _method=PUT
         agar Laravel tahu ini request UPDATE bukan CREATE --}}
    <form action="{{ route('admin.events.update', $event->id) }}" method="POST"
        ...
        {{-- JUDUL
             ...
             - Pertama buka = isi dari database ($event->title)
             - Validasi gagal = isi dari input user --}}
        <div>
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl
                             focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600
                             outline-none transition font-medium resize-none">{{ old('description', $event->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1.5 font-medium">⚠ {{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                {{-- $event->date sudah Carbon (karena $casts di Model)
                     format Y-m-d\TH:i = format yang diterima input datetime-local HTML
                     contoh output: 2026-05-10T19:00 --}}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Kapasitas Tiket</label>
        ...
        <div>
            <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">
                Poster Event
                <span class="text-slate-400 font-normal normal-case text-xs">— Kosongkan jika tidak ingin mengganti</span>
            </label>

            {{-- Tampilkan poster yang sudah tersimpan di database --}}
            @if($event->poster_path)
                <div class="mb-3 flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">
                            Pilih file baru di bawah untuk mengganti poster ini.
                        </p>
                    </div>
                </div>
            @endif

            {{-- Area upload poster baru --}}
            <input type="file" name="poster" id="poster_edit" accept="image/*"
                   class="hidden" onchange="previewImage(this, 'preview_edit', 'placeholder_edit')">

            <label for="poster_edit"
                   class="flex flex-col items-center justify-center gap-3 w-full h-40
                ...
                <div id="placeholder_edit" class="flex flex-col items-center gap-2">
                    <svg class="w-9 h-9 text-slate-300 group-hover:text-indigo-400 transition"
                    ...
                    <p class="text-sm font-semibold text-slate-400 group-hover:text-indigo-500">
                        {{ $event->poster_path ? 'Klik untuk ganti poster' : 'Klik untuk pilih gambar' }}
                    </p>
                </div>

                <img id="preview_edit" src="#" alt="Preview poster baru"
